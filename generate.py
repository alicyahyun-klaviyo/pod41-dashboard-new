"""
generate.py — fetches #support-pod-41-ctrl-alt-elite from Slack and writes index.html

Requirements:
  pip install requests

Environment variables needed:
  SLACK_TOKEN  — Slack Bot or User OAuth token with channels:history + channels:read scopes
"""
...
CHANNEL_ID = "C06MP0YHY58"
SLACK_TOKEN = os.environ["SLACK_TOKEN"]
HEADERS = {"Authorization": f"Bearer {SLACK_TOKEN}"}

# Earliest timestamp to fetch (1 Jan 2026 UTC)
OLDEST_TS = "1735689600"

# Test/dummy messages to skip
SKIP_KAS = {"45678", "123456", "TEST", "ticket link"}


def fetch_all_messages():
    """Paginate through conversations.history and return all messages."""
    messages = []
    cursor = None
    page = 1

    while True:
        params = {"channel": CHANNEL_ID, "limit": 200, "oldest": OLDEST_TS}
        if cursor:
        ...
        if not data.get("ok"):
            raise RuntimeError(f"Slack API error: {data.get('error')} — make sure your token has channels:history scope and the bot is in the channel.")

        batch = data.get("messages", [])
        messages.extend(batch)
        print(f"  Page {page}: fetched {len(batch)} messages (total so far: {len(messages)})")
...
def clean_text(text):
    """Strip Slack link markup like <url|label> → label, <url> → ''"""
    text = re.sub(r"<([^|>]+)\|([^>]+)>", r"\2", text)
    text = re.sub(r"<[^>]+>", "", text)
    return text.strip()


def extract_field(text, field_name, stop_fields=None):
    """Extract text after *field_name* up to the next bold field."""
    stop_fields = stop_fields or [
        "KA", "Ticket Link", "Product Area", "Issue Summary",
        "Relevant Links", "What have you tried so far",
    ]
    # Build stop pattern
...
def parse_message(msg):
    ...
    if "needs help!" not in text:
        return None

    # Requester name
    req_m = re.search(r"<@[^|]+\|([^>]+)> needs help!", text)
    if not req_m:
        return None

    ka_raw = extract_field(text, "KA")
    ka = ka_raw.strip()
    if not ka or ka in SKIP_KAS:
        return None

    area = clean_text(extract_field(text, "Product Area"))
    if not area or area == "-":
        return None

    summary_raw = extract_field(text, "Issue Summary")
    summary = clean_text(summary_raw.replace("\n", " "))[:400]

    tried_raw = extract_field(text, "What have you tried so far")
    tried = parse_tried(tried_raw)

    ticket_raw = extract_field(text, "Ticket Link")
    ticket_url = ""
    link_m = re.search(r"<([^|>]+)(?:\|[^>]+)?>", ticket_raw)
    if link_m:
        ticket_url = link_m.group(1)
...
def build_html(questions, generated_at):
    ...
    last_updated_label = f"{date_range} · {count} questions · refreshed {generated_at}"
...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  th.sorted .sort-icon {{ color: #4A154B; }}
  ...
  .expand-content {{ padding: 16px 18px; display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }}
  ...
  .expand-section li {{ font-size: 13px; color: #3a3a3c; line-height: 1.5; margin-bottom: 2px; }}
...
<header>
  <div class="header-left">
    ...
    <div>
      ...
      <div class="last-updated" id="lastUpdated">{last_updated_label}</div>
    </div>
  </div>
</header>
<div class="main">
  ...
  <div class="filter-bar">
    <div class="search-wrap">
      ...
      <input type="text" id="searchInput" placeholder="Search issue, product area, KA, requester…" oninput="renderTable()">
    </div>
...
function getFiltered(){{
  ...
  return QUESTIONS.filter(q=>{{
    ...
    if(s){{const hay=[q.requester,q.area,q.summary,q.ka,...q.tried].join(" ").toLowerCase();if(!hay.includes(s))return false;}}
    return true;
  }});
}}
...
function renderTable(){{
  ...
  if(!filtered.length){{container.innerHTML='<div class="empty-state">No questions match your filters.</div>';return;}}
  let html=`<table><thead><tr>
    <th onclick="handleSort('date')" class="${{sortCol==="date"?"sorted":""}}">Date ${{si("date")}}</th>
    <th onclick="handleSort('requester')" class="${{sortCol==="requester"?"sorted":""}}">Requester ${{si("requester")}}</th>
    <th>KA</th>
    <th onclick="handleSort('area')" class="${{sortCol==="area"?"sorted":""}}">Product Area ${{si("area")}}</th>
    <th>Issue Summary</th>
    <th>Ticket</th>
  </tr></thead><tbody>`;
  filtered.forEach(q=>{{
    const origIdx=QUESTIONS.indexOf(q);
    const isExp=expandedIdx===origIdx;
    html+=`<tr onclick="toggleExpand(${{origIdx}})" class="${{isExp?"expanded-row":""}}">
      <td class="td-date">${{fmtDate(q.date)}}</td>
      <td class="td-requester">${{esc(q.requester)}}</td>
      <td class="td-ka">${{esc(q.ka)}}</td>
      <td><span class="area-tag">${{esc(q.area)}}</span></td>
      <td class="td-summary"><div class="td-summary-text">${{esc(q.summary)}}</div></td>
      <td>${{q.ticket_url?`<a class="ticket-link" href="${{esc(q.ticket_url)}}" target="_blank" onclick="event.stopPropagation()">View ↗</a>`:'<span style="color:#c7c7cc;font-size:12px">–</span>'}}</td>
    </tr>`;
    if(isExp){{
      html+=`<tr class="expand-row"><td colspan="6"><div class="expand-content">
        ...
        <div class="expand-section"><h4>Details</h4>
          <p><strong>KA:</strong> ${{esc(q.ka)}}</p>
          <p><strong>Requester:</strong> ${{esc(q.requester)}}</p>
          <p><strong>Date:</strong> ${{fmtDate(q.date)}}</p>
          ${{q.has_files?'<p><strong>Attachments:</strong> ✓ Files attached</p>':''}}
          ${{q.ticket_url?`<p><strong>Ticket:</strong> <a class="ticket-link" href="${{esc(q.ticket_url)}}" target="_blank">Open in Zendesk ↗</a></p>`:''}}
        </div>
      </div></td></tr>`;
    }}
  }});
...
def main():
    ...
    print(f"Parsed {len(questions)} help requests")

    # Generated-at timestamp in AEST
    aest = timezone(timedelta(hours=10))
    now = datetime.now(tz=aest)
    generated_at = now.strftime("%-d %b %Y %I:%M %p AEST")

    html = build_html(questions, generated_at)
